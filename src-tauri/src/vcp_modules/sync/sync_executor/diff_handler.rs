...
use crate::vcp_modules::sync_service::{emit_sync_log, fail_sync_session, SyncCommand};
...
async fn cancel_sync_with_error(
    ...
    fail_sync_session(app_handle, &message).await;
    let _ = tx_internal.send(SyncCommand::Cancel);
}
    ...
    pub async fn handle_diff(
        ...
        for item in items_clone {
            ...
            if id.is_empty()
                || !matches!(
                    action.as_str(),
                    "PULL" | "PUSH" | "DELETE" | "PUSH_DELETE" | "SKIP"
                )
            {
                return Err(format!(
                    "Malformed {} diff item: id={:?}, action={:?}",
                    data_type, id, action
                ));
            }
        ...
        if !batch_pull_requests.is_empty() {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                for chunk in batch_pull_requests.chunks(chunk_size) {
                    ...
                    .await
                    {
                        cancel_sync_with_error(
                            &h_in,
                            &tx_internal_in,
                            format!("Batch pull failed for {} entities: {}", sub_count, error),
                        )
                        .await;
                        return;
                    }
        ...
        if !push_topics_to_fetch.is_empty() {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                // 异步批量查询 Topic 元数据
                for (id, _diff_owner_id, _owner_type) in push_topics_to_fetch {
                    log::debug!("[SyncDebug] Fetching metadata for topic: {}", id);
                    let row_res = sqlx::query("SELECT topic_id, title, created_at, locked, unread, owner_id, owner_type \
                    ...
                    match row_res {
                        Ok(Some(r)) => {
                            ...
                            log::debug!("[SyncDebug] Found topic {} (owner: {})", tid, db_owner_id);
                        ...
                        Err(e) => {
                            cancel_sync_with_error(
                                &h_in,
                                &tx_internal_in,
                                format!("Failed to load topic {} for sync push: {}", id, e),
                            )
                            .await;
                            return;
                        }
                ...
                for chunk in batch_push_requests.chunks(1000) {
                    ...
                    .await;
                    if let Err(error) = push_res {
                        cancel_sync_with_error(
                            &h_in,
                            &tx_internal_in,
                            format!("Failed to push topic metadata batch: {}", error),
                        )
                        .await;
                        return;
                    }
                        ...
                        .for_each_concurrent(15, |item| {
                            ...
                            async move {
                                ...
                                let operation_result = match action.as_str() {
                                    "PULL" => match &data_type_task {
                                        SyncDataType::Avatar => {
                                            ...
                                            if parts.len() == 2 {
                                                ...
                                                Err(format!("Invalid avatar pull id: {}", id))
                                            }
                                        }
                                        SyncDataType::Agent => {
                                    ...
                                    "PUSH" => match &data_type_task {
                                        SyncDataType::Agent => PushExecutor::push_agent(
                                            &h_task, &c_task, &b_task, &token_task, &id,
                                        )
                                        .await,
                                        SyncDataType::Group => PushExecutor::push_group(
                                            &h_task, &c_task, &b_task, &token_task, &id,
                                        )
                                        .await,
                                        SyncDataType::Avatar => {
                                            ...
                                            if parts.len() == 2 {
                                                ...
                                                Err(format!("Invalid avatar push id: {}", id))
                                            }
                                        }
                                        _ => {
                                            should_decrement = false;
                                    ...
                                    "DELETE" | "PUSH_DELETE" => {
                                        ...
                                        let result = match &data_type_task {
                                            SyncDataType::Agent => {
                                            ...
                                            SyncDataType::Message => {
                                                DeleteExecutor::soft_delete_message(&h_task, &id).await
                                            }
                                        };
                                        if result.is_ok() && action == "PUSH_DELETE" {
                                            if let Err(error) = tx_internal_task
                                ...
                                if let Err(error) = operation_result {
                                    cancel_sync_with_error(
                                        ...
                                        format!(
                                            "Failed to apply {} {} for {}: {}",
                                            data_type_task, action, id, error
                                        ),
                                    )
                                    .await;
                                    return;
