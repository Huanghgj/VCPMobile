  ...
  if (gestureMode.value === "pan") {
    ...
    if (event.touches.length >= 2) {
      ...
      if (touchStartDist > 20 && containerRef.value) {
        ...
        const targetX =
          midX - vcx - factor * (touchStartMidX - vcx - touchStartPanX);
        const targetY =
          midY - vcy - factor * (touchStartMidY - vcy - touchStartPanY);
...
async function sourceForNativeSave(src: string): Promise<string> {
...
async function trySaveViaWebViewFetch(
  src: string,
  fileName: string,
): Promise<void> {
  if (src.startsWith("data:")) throw new Error("already data url");
  const response = await fetch(src);
...
async function fallbackBrowserDownload(
  src: string,
  fileName: string,
): Promise<void> {
