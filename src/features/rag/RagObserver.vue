...
type FilterValue = "all" | "rag" | "chain" | "chat" | "memo" | "dream";
...
const handleTouchMove = (e: TouchEvent) => {
  ...
  if (absX > 50 && absY <= absX * 0.4663) {
    ...
    const currentIdx = filterTabs.findIndex(
      (tab) => tab.value === activeFilter.value
    );
    if (currentIdx !== -1) {
      if (deltaX > 0) {
        // 向右滑，切换至上一个选项卡
...
watch(activeFilter, async (newVal) => {
  ...
  const activeBtn = Array.from(
    container.querySelectorAll<HTMLElement>("[data-tab-value]")
  ).find((el) => el.dataset.tabValue === newVal);
  if (activeBtn) {
...
const getAccentClass = (type: string) => {
  ...
  if (type === "AI_MEMO_RETRIEVAL" || type === "DailyNote")
...
watch(
  () => store.triggerSpectrumAnimation,
  (isAnimating) => {
    if (!props.isOpen || document.hidden) return;
    if (isAnimating) {
      drawSpectrum();
...
onUnmounted(() => {
  document.removeEventListener(
    "visibilitychange",
    handleSpectrumVisibilityChange
  );
  store.destroyListener();
  stopSpectrum();
});
...
<template>
  <SlidePage :is-open="props.isOpen" :z-index="props.zIndex">
    <div
      class="relative w-full h-full flex flex-col overflow-hidden z-10"
      :class="[
        themeStore.isDarkResolved
          ? 'bg-[#131922] text-slate-100'
          : 'bg-[#e8ecef] text-slate-800',
        { 'pointer-events-none': !props.isOpen },
      ]"
    >
      <!-- 头部状态栏 -->
      <div
        class="flex items-center justify-between px-4 pt-[calc(env(safe-area-inset-top)+8px)] pb-3 border-b relative z-10"
        :class="
          themeStore.isDarkResolved
            ? 'bg-[#1a202c] border-white/5 text-slate-300'
            : 'bg-[#edf0f2] border-gray-300/60 shadow-sm text-slate-700'
        "
      >
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-2">
            ...
            <span
              ...
              :class="
                themeStore.isDarkResolved ? 'text-white/70' : 'text-gray-500'
              "
              >{{ statusLabel }}</span
            >
          </div>
        ...
        <div class="flex items-center gap-2">
          <span
            class="text-xs font-bold tracking-wider flex items-center gap-1"
            :class="
              themeStore.isDarkResolved ? 'text-white/50' : 'text-gray-600'
            "
          >
        ...
        <canvas
          ref="spectrumCanvas"
          class="absolute left-0 bottom-0 w-full h-[6px] block opacity-80 pointer-events-none"
        ></canvas>
      </div>
      ...
      <div
        ref="tabContainerRef"
        class="flex gap-2 px-3 py-2.5 overflow-x-auto no-scrollbar border-b bg-transparent relative z-10"
        :class="
          themeStore.isDarkResolved ? 'border-white/5' : 'border-gray-300/40'
        "
      >
        <button
      ...
      >
        <!-- 空白占位 -->
        <div
          v-if="filteredMetadataList.length === 0"
          class="flex flex-col items-center justify-center py-20"
          :class="
            themeStore.isDarkResolved ? 'text-white/20' : 'text-gray-400/50'
          "
        >
          <Sparkles
            :size="32"
            class="mb-4 stroke-[1.5]"
            :class="
              themeStore.isDarkResolved ? 'text-white/10' : 'text-gray-300'
            "
          />
          <span class="text-xs tracking-wider">暂无灵视认知广播数据</span>
          <span class="text-[9px] mt-1 opacity-50"
            >等待 AI 进行思考或检索...</span
          >
        </div>
          ...
          >
            ...
            >
              <div class="flex-1 min-w-0 pr-2">
                <div class="flex items-center justify-between mb-1.5">
                  <div class="flex items-center gap-1.5 min-w-0">
                    <component
                      ...
                      :class="
                        themeStore.isDarkResolved
                          ? getTitleStyle(item.type).colorClass
                          : getTitleStyle(item.type).colorClass.replace(
                              '-400',
                              '-600'
                            )
                      "
                  ...
                  <span
                    class="text-[9px] font-mono shrink-0 ml-2"
                    :class="
                      themeStore.isDarkResolved
                        ? 'text-white/30'
                        : 'text-gray-400'
                ...
                <div
                  v-if="item.subtitle"
                  class="text-[9px] font-mono font-bold tracking-wider mb-1"
                  :class="
                    themeStore.isDarkResolved
                      ? 'text-blue-400/80'
                      : 'text-blue-600'
              ...
              <div
                ...
                :class="
                  themeStore.isDarkResolved ? 'text-white/30' : 'text-gray-400'
                "
              >
            ...
            <div
              ...
              class="border-t p-3 text-[11px]"
              :class="
                themeStore.isDarkResolved
                  ? 'border-white/5 bg-[#111620]'
                  : 'border-gray-300/40 bg-[#e9ecef]'
              "
            >
              <!-- 1. 加载中 -->
              <div
                v-if="payloadLoading[item.id]"
              ...
              <!-- 2. 加载失败 -->
              <div
                v-else-if="!payloadCache[item.id]"
                class="flex flex-col items-center justify-center py-4 gap-2 text-red-500"
              >
              ...
              <template v-else>
                <div
                  class="flex justify-between items-center mb-3 pb-2 border-b"
                  :class="
                    themeStore.isDarkResolved
                      ? 'border-white/5'
                      : 'border-gray-300/50'
                ...
                >
                  ...
                  >
                    <Check
                      v-if="copiedCardId === item.id"
                      :size="10"
                      class="text-green-500"
                    />
                    <Copy v-else :size="10" />
                    <span>{{
                      copiedCardId === item.id ? "已复制" : "复制完整JSON"
                    }}</span>
                  </button>
                </div>
                ...
                <div
                  v-if="
                    item.type === '' || item.type === 'RAG_RETRIEVAL_DETAILS'
                  "
                  class="space-y-2.5"
                >
                  <!-- RAG Query Section -->
                  <div
                    ...
                    :ref="
                      (el) => {
                        if (el) subCardRefs[`${item.id}_query`] = el;
                      }
                    "
                  ...
                  >
                    <div
                      class="flex justify-between items-center mb-1 text-[9px] font-bold"
                      :class="
                        themeStore.isDarkResolved
                          ? 'text-blue-400/80'
                          : 'text-blue-600'
                    ...
                    >
                      ...
                      <div
                        v-if="payloadCache[item.id].query.length > 30"
                        class="flex items-center gap-0.5 scale-90 select-none"
                        :class="
                          themeStore.isDarkResolved
                            ? 'text-blue-400/60'
                            : 'text-blue-500'
                      ...
                      >
                        <span>{{
                          expandedSubCardIds.has(`${item.id}_query`)
                            ? "收起"
                            : "展开"
                        }}</span>
                        <ChevronUp
                          v-if="expandedSubCardIds.has(`${item.id}_query`)"
                          :size="10"
                        />
                    ...
                    <div
                      class="leading-relaxed font-mono text-[10px] select-text"
                    >
                      <template
                        v-if="
                          payloadCache[item.id].query.length > 30 &&
                          !expandedSubCardIds.has(`${item.id}_query`)
                        "
                      >
                      ...
                      <template v-else>
                        <RagPayloadDetail
                          :class="[
                            themeStore.isDarkResolved
                              ? 'prose-invert text-white/80'
                              : 'text-gray-800',
                            'prose prose-xs text-[10px]',
                          ]"
                          :text="payloadCache[item.id].query"
                          :is-query="true"
                        />
                      </template>
                    </div>
                  ...
                  <div
                    v-if="
                      payloadCache[item.id].coreTags &&
                      payloadCache[item.id].coreTags.length > 0
                    "
                  ...
                  >
                    <span
                      ...
                      :class="
                        themeStore.isDarkResolved
                          ? 'bg-[#b38c2b]/90 shadow-[0_0_6px_rgba(179,140,43,0.2)]'
                          : 'bg-[#b38c2b] shadow-sm'
                      "
                    >
                      <Sparkles
                        :size="9"
                        class="text-yellow-100 fill-yellow-100 shrink-0"
                      />
                      {{ tag }}
                    </span>
                  </div>
                  ...
                  <div
                    class="flex flex-wrap gap-1.5 mb-2 text-[9px] font-mono"
                    :class="
                      themeStore.isDarkResolved
                        ? 'text-white/40'
                        : 'text-gray-500'
                  ...
                  >
                    <span
                      v-if="payloadCache[item.id].tagStats"
                      class="px-1.5 py-0.5 rounded border"
                      :class="
                        themeStore.isDarkResolved
                          ? 'bg-yellow-500/10 border-yellow-500/20 text-yellow-500/80'
                          : 'bg-yellow-50 border-yellow-200 text-yellow-700'
                      "
                    >
                      🏷️ 匹配标签:
                      {{
                        payloadCache[item.id].tagStats.uniqueMatchedTags
                          ?.length || 0
                      }}个 | Boost均值:
                      {{ payloadCache[item.id].tagStats.avgBoostFactor }}
                    </span>
                    <span
                      v-if="
                        payloadCache[item.id].timeRanges &&
                        payloadCache[item.id].timeRanges.length > 0
                      "
                      class="px-1.5 py-0.5 rounded border"
                      :class="
                        themeStore.isDarkResolved
                    ...
                    >
                      📅 时间窗口:
                      {{
                        payloadCache[item.id].timeRanges[0].start.slice(0, 10)
                      }}
                      ➜
                      {{ payloadCache[item.id].timeRanges[0].end.slice(0, 10) }}
                    </span>
                  </div>

                  <div
                    class="text-[9px] font-bold uppercase tracking-widest mb-1.5 flex items-center gap-1"
                    :class="
                      themeStore.isDarkResolved
                        ? 'text-white/20'
                        : 'text-gray-400'
                  ...
                  >
                    召回结果列表 ({{
                      payloadCache[item.id].results?.length || 0
                    }})
                  </div>
                    ...
                    <div
                      ...
                      :class="
                        themeStore.isDarkResolved
                          ...
                          : 'bg-[#f1f3f5] border-gray-350/50 shadow-sm text-slate-800'
                      "
                    >
                      ...
                      >
                        <div class="flex justify-between items-center">
                          <div class="flex items-center gap-1.5 min-w-0 pr-2">
                            ...
                            <span
                              class="font-mono text-[8px] px-1 py-0.5 rounded truncate"
                              :class="
                                themeStore.isDarkResolved
                                  ? 'bg-white/5'
                                  : 'bg-gray-250 text-gray-600'
                              "
                              >来源: {{ res.source || "Unknown" }}</span
                            >
                          </div>
                          <div
                            :class="
                          ...
                          >
                            <ChevronUp
                              v-if="
                                expandedSubCardIds.has(`${item.id}_res_${idx}`)
                              "
                              :size="12"
                            />
                        ...
                        <div
                          v-if="
                            !expandedSubCardIds.has(`${item.id}_res_${idx}`)
                          "
                          class="text-[9px] truncate mt-1 pl-1 font-mono leading-normal"
                          :class="
                            themeStore.isDarkResolved
                              ? 'text-white/30'
                              : 'text-gray-400'
                      ...
                      <div
                        v-if="expandedSubCardIds.has(`${item.id}_res_${idx}`)"
                      ...
                      >
                        ...
                        <div
                          v-if="res.matchedTags && res.matchedTags.length > 0"
                          class="pt-1.5 border-t font-mono font-bold text-[9px] leading-relaxed"
                          :class="
                            themeStore.isDarkResolved
                ...
                <!-- 元思考链详情 -->
                <div
                  v-else-if="item.type === 'META_THINKING_CHAIN'"
                  class="space-y-3"
                >
                  <!-- Flow path -->
                  <div
                    class="p-2 rounded border font-mono text-[9px]"
                    :class="
                      themeStore.isDarkResolved
                        ? 'bg-purple-900/10 border-purple-500/10 text-purple-300'
                  ...
                  >
                    ...
                    <div
                      class="flex flex-wrap items-center gap-1 leading-normal"
                    >
                      <template
                        v-for="(stage, idx) in payloadCache[item.id].stages"
                        :key="stage.stage"
                      >
                        <span
                          ...
                          :class="
                            themeStore.isDarkResolved
                              ...
                              : 'bg-purple-100/60'
                          "
                          >{{ stage.clusterName }}</span
                        >
                        <span
                          v-if="idx < payloadCache[item.id].stages.length - 1"
                          :class="
                            themeStore.isDarkResolved
                              ? 'text-white/20'
                              : 'text-gray-300'
                  ...
                  <div
                    ...
                    :ref="
                      (el) => {
                        if (el) subCardRefs[`${item.id}_chain_query`] = el;
                      }
                    "
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <span>{{
                          expandedSubCardIds.has(`${item.id}_chain_query`)
                            ? "收起"
                            : "展开"
                        }}</span>
                        <ChevronUp
                  ...
                  <div
                    v-if="
                      payloadCache[item.id].activatedGroups &&
                      payloadCache[item.id].activatedGroups.length > 0
                    "
                    class="flex flex-wrap gap-1 mb-1.5"
                  >
                    <span
                      v-for="grp in payloadCache[item.id].activatedGroups"
                      :key="grp"
                      class="px-1.5 py-0.5 rounded border text-[8px] font-mono"
                      :class="
                        themeStore.isDarkResolved
                  ...
                  <div class="space-y-2.5">
                    ...
                    >
                      <div
                        class="text-[10px] font-bold flex justify-between items-center"
                        :class="
                          themeStore.isDarkResolved
                            ? 'text-purple-200'
                            : 'text-purple-850'
                      ...
                      >
                        <span
                          >阶段 {{ stage.stage }}: {{ stage.clusterName }}</span
                        >
                        <span
                      ...
                      <div class="space-y-1.5">
                        ...
                        >
                          ...
                          <div
                            @click.stop="
                              toggleSubCard(
                                `${item.id}_s${stage.stage}_res_${rIdx}`
                              )
                            "
                            ...
                            <RagPayloadDetail
                              :class="[
                                themeStore.isDarkResolved
                                  ? 'prose-invert text-white/85'
                                  : 'text-gray-800',
                                'prose prose-xs leading-relaxed break-words font-mono text-[10px] select-text',
                              ]"
                              :text="res.text"
                            />
                          </div>
                ...
                <!-- Agent 私聊 -->
                <div
                  v-else-if="item.type === 'AGENT_PRIVATE_CHAT_PREVIEW'"
                  class="space-y-2.5"
                >
                  <div
                    class="p-2.5 rounded border space-y-2"
                    :class="
                      themeStore.isDarkResolved
                        ? 'bg-white/5 border-white/5'
                        : 'bg-[#f1f3f5] border-gray-300/60 shadow-sm'
                    "
                  >
                    <!-- USER QUERY Box -->
                    <div
                      ...
                      :ref="
                        (el) => {
                          if (el) subCardRefs[`${item.id}_chat_q`] = el;
                        }
                      "
                    ...
                    >
                      <div
                        class="flex justify-between items-center mb-1 text-[8px] font-bold font-mono tracking-wider uppercase"
                        :class="
                          themeStore.isDarkResolved
                            ? 'text-white/35'
                            : 'text-gray-400'
                      ...
                      >
                        ...
                        <div
                          v-if="payloadCache[item.id].query.length > 30"
                          class="flex items-center gap-0.5 scale-90 select-none font-normal normal-case"
                          :class="
                            themeStore.isDarkResolved
                              ? 'text-white/40'
                              : 'text-gray-500'
                        ...
                        >
                          <span>{{
                            expandedSubCardIds.has(`${item.id}_chat_q`)
                              ? "收起"
                              : "展开"
                          }}</span>
                          <ChevronUp
                            v-if="expandedSubCardIds.has(`${item.id}_chat_q`)"
                            :size="10"
                          />
                          <ChevronDown v-else :size="10" />
                        </div>
                    ...
                    <div
                      ...
                      :ref="
                        (el) => {
                          if (el) subCardRefs[`${item.id}_chat_r`] = el;
                        }
                      "
                    ...
                    >
                      <div
                        class="flex justify-between items-center mb-1 text-[8px] font-bold text-orange-400 font-mono tracking-wider uppercase"
                      >
                        <span>AI INNER VOICE</span>
                        <div
                          v-if="payloadCache[item.id].response.length > 30"
                          class="flex items-center gap-0.5 scale-90 select-none font-normal normal-case"
                          :class="
                            themeStore.isDarkResolved
                              ? 'text-orange-400/60'
                        ...
                        >
                          <span>{{
                            expandedSubCardIds.has(`${item.id}_chat_r`)
                              ? "收起"
                              : "展开"
                          }}</span>
                          <ChevronUp
                            v-if="expandedSubCardIds.has(`${item.id}_chat_r`)"
                            :size="10"
                          />
                          <ChevronDown v-else :size="10" />
                        </div>
                      ...
                      >
                        <template
                          v-if="
                            payloadCache[item.id].response.length > 30 &&
                            !expandedSubCardIds.has(`${item.id}_chat_r`)
                          "
                        >
                        ...
                        <template v-else>
                          <RagPayloadDetail
                            ...
                            :text="payloadCache[item.id].response"
                            :is-query="true"
                          />
                        </template>
                      </div>
                ...
                <!-- 记忆检索 (AIMemo) -->
                <div
                  v-else-if="item.type === 'AI_MEMO_RETRIEVAL'"
                  class="space-y-2.5"
                >
                  <!-- Meta pills -->
                  <div
                    class="flex flex-wrap gap-1.5 mb-1.5 text-[9px] font-mono"
                  >
                    <span
                      class="px-1.5 py-0.5 rounded border"
                      :class="
                    ...
                    >
                      模式:
                      {{ payloadCache[item.id].mode || "aggregated_single" }}
                    </span>
                    <span
                      v-if="payloadCache[item.id].fileCount"
                      class="px-1.5 py-0.5 rounded border"
                      :class="
                        themeStore.isDarkResolved
                          ? 'bg-green-500/10 border-green-500/20 text-green-400'
                    ...
                    <span
                      v-if="
                        payloadCache[item.id].dbNames &&
                        payloadCache[item.id].dbNames.length > 0
                      "
                      class="px-1.5 py-0.5 rounded border"
                      :class="
                        themeStore.isDarkResolved
                  ...
                  <div
                    ...
                    :ref="
                      (el) => {
                        if (el) subCardRefs[`${item.id}_memo_query`] = el;
                      }
                    "
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <span>{{
                          expandedSubCardIds.has(`${item.id}_memo_query`)
                            ? "收起"
                            : "展开"
                        }}</span>
                        <ChevronUp
                          v-if="expandedSubCardIds.has(`${item.id}_memo_query`)"
                          :size="10"
                        />
                        <ChevronDown v-else :size="10" />
                      </div>
                  ...
                  >
                    <div
                      class="text-[9px] font-bold mb-1 flex items-center gap-1"
                      :class="
                        themeStore.isDarkResolved
                          ? 'text-green-400'
                          : 'text-green-700'
                ...
                <div v-else-if="item.type === 'DailyNote'" class="space-y-2.5">
                  <div
                    ...
                    :ref="
                      (el) => {
                        if (el) subCardRefs[`${item.id}_note`] = el;
                      }
                    "
                  ...
                  >
                    <div
                      class="flex justify-between items-center mb-1 text-[10px] font-bold"
                      :class="
                        themeStore.isDarkResolved
                          ? 'text-green-400'
                          : 'text-green-700'
                    ...
                    >
                      ...
                      <div
                        v-if="payloadCache[item.id].message.length > 30"
                        class="flex items-center gap-0.5 scale-90 select-none font-normal"
                        :class="
                          themeStore.isDarkResolved
                            ? 'text-green-400/60'
                            : 'text-green-600/80'
                      ...
                      >
                        <span>{{
                          expandedSubCardIds.has(`${item.id}_note`)
                            ? "收起"
                            : "展开"
                        }}</span>
                        <ChevronUp
                          v-if="expandedSubCardIds.has(`${item.id}_note`)"
                          :size="10"
                        />
                        <ChevronDown v-else :size="10" />
                      </div>
                    ...
                    <div
                      class="leading-relaxed font-mono text-[10px] break-words select-text"
                      :class="
                        themeStore.isDarkResolved
                          ? 'text-white/80'
                          : 'text-gray-800'
                    ...
                    >
                      <template
                        v-if="
                          payloadCache[item.id].message.length > 30 &&
                          !expandedSubCardIds.has(`${item.id}_note`)
                        "
                      >
                      ...
                      <template v-else>
                        <RagPayloadDetail
                          ...
                          :text="payloadCache[item.id].message"
                          :is-query="true"
                        />
                      </template>
                    </div>
                ...
                <!-- Agent 做梦联想 -->
                <div
                  v-else-if="item.type === 'AGENT_DREAM_ASSOCIATIONS'"
                  class="space-y-3"
                >
                  <!-- dreamId 标签 -->
                  <div
                    v-if="payloadCache[item.id].dreamId"
                    class="text-[9px] font-mono mb-1.5"
                    :class="
                      themeStore.isDarkResolved
                        ? 'text-white/40'
                        : 'text-gray-550'
                  ...
                  <div class="flex flex-wrap gap-1.5 mb-1 text-[9px] font-mono">
                    ...
                    >
                      联想唤醒:
                      {{ payloadCache[item.id].associationCount || 0 }} 篇
                    </span>
                  </div>
                  ...
                  <div
                    v-if="
                      payloadCache[item.id].seeds &&
                      payloadCache[item.id].seeds.length > 0
                    "
                  >
                    <div
                      class="text-[9px] font-bold uppercase tracking-widest mb-1.5 flex items-center gap-1"
                      ...
                      <div
                        ...
                        :class="
                          themeStore.isDarkResolved
                            ...
                            : 'bg-[#f1f3f5] border-gray-300/50 text-gray-700'
                        "
                      >
                        <div
                        ...
                        <div
                          class="truncate text-[8px] mt-0.5"
                          :class="
                            themeStore.isDarkResolved
                              ? 'text-white/40'
                              : 'text-gray-500'
                  ...
                  <div
                    v-if="
                      payloadCache[item.id].associations &&
                      payloadCache[item.id].associations.length > 0
                    "
                    class="mt-2.5"
                  >
                    <div
                    ...
                    <div class="space-y-1">
                      <div
                        v-for="(assoc, idx) in payloadCache[item.id]
                          .associations"
                        :key="idx"
                        ...
                        :class="
                          themeStore.isDarkResolved
                            ...
                            : 'bg-[#f1f3f5] border-gray-300/50 text-gray-600'
                        "
                      >
                        <span
                          class="truncate pr-2"
                        ...
                        >
                        <span
                          class="shrink-0 font-bold text-[8px]"
                          :class="
                            themeStore.isDarkResolved
                              ? 'text-yellow-400'
                              : 'text-yellow-600'
                ...
                <!-- Agent 做梦整理操作 -->
                <div
                  v-else-if="item.type === 'AGENT_DREAM_OPERATIONS'"
                  class="space-y-3"
                >
                  <!-- dreamId 标签 -->
                  <div
                  ...
                  <!-- meta info -->
                  <div
                    class="flex justify-between items-center text-[9px] font-mono pb-1.5 border-b"
                    :class="
                      themeStore.isDarkResolved
                        ? 'text-white/40 border-white/5'
                        : 'text-gray-550 border-gray-300/60'
                    "
                  >
                    <span
                      >整理日志:
                      {{ payloadCache[item.id].logFile || "None" }}</span
                    >
                    <span
                      >操作数量:
                      {{ payloadCache[item.id].operations?.length || 0 }}
                      项</span
                    >
                  </div>
                    ...
                    >
                      <div class="flex items-center gap-1.5 min-w-0 pr-2">
                        ...
                        <span
                          ...
                          :class="{
                            'bg-orange-500/25 text-orange-400 border border-orange-500/35':
                              op.type === 'merge',
                            'bg-red-500/25 text-red-400 border border-red-500/35':
                              op.type === 'delete',
                            'bg-blue-500/25 text-blue-400 border border-blue-500/35':
                              op.type === 'insight',
                            'bg-gray-500/25 text-gray-400 border border-gray-500/35':
                              op.type !== 'merge' &&
                              op.type !== 'delete' &&
                              op.type !== 'insight',
                          }"
                ...
                <!-- Agent 梦叙事 -->
                <div
                  v-else-if="item.type === 'AGENT_DREAM_NARRATIVE'"
                  class="space-y-2.5 select-text font-mono"
                >
                  <!-- dreamId 标签 -->
                  <div
                    v-if="payloadCache[item.id].dreamId"
                  ...
                  <div
                    ...
                    :ref="
                      (el) => {
                        if (el) subCardRefs[`${item.id}_narrative`] = el;
                      }
                    "
                  ...
                  >
                    <div
                      class="flex justify-between items-center mb-1.5 text-[9px] font-bold"
                      :class="
                        themeStore.isDarkResolved
                          ? 'text-pink-400'
                          : 'text-pink-600'
                    ...
                    >
                      ...
                      <div
                        v-if="
                          payloadCache[item.id].narrative &&
                          payloadCache[item.id].narrative.length > 50
                        "
                        class="flex items-center gap-0.5 scale-90 select-none"
                        :class="
                          themeStore.isDarkResolved
                      ...
                      >
                        <span>{{
                          expandedSubCardIds.has(`${item.id}_narrative`)
                            ? "收起"
                            : "展开"
                        }}</span>
                        <ChevronUp
                          v-if="expandedSubCardIds.has(`${item.id}_narrative`)"
                          :size="10"
                        />
                        <ChevronDown v-else :size="10" />
                      </div>
                    ...
                    <div class="leading-relaxed break-words text-[10px]">
                      ...
                      <template v-else>
                        <RagPayloadDetail
                          ...
                          :text="payloadCache[item.id].narrative || ''"
                        />
                      </template>
                    </div>
                  </div>
                ...
                <!-- Agent 梦境开始与结束 -->
                <div
                  v-else-if="item.type === 'AGENT_DREAM_START'"
                  class="space-y-2.5"
                >
                  <div
                    class="p-2.5 rounded border-l-2 flex items-center gap-2"
                    :class="
                      themeStore.isDarkResolved
                  ...
                  >
                    ...
                    <div class="font-mono text-[10px]">
                      {{ payloadCache[item.id].message }}
                    </div>
                  </div>
                </div>

                <div
                  v-else-if="item.type === 'AGENT_DREAM_END'"
                  class="space-y-2.5"
                >
                  <div
                    class="p-2.5 rounded border-l-2 flex flex-col gap-1.5"
                    :class="
                      payloadCache[item.id].status === 'error'
                        ? themeStore.isDarkResolved
                          ? 'bg-red-500/10 border-red-500/20 border-l-red-500 text-red-400'
                          : 'bg-red-100/30 border-red-200/80 border-l-red-500 text-red-700 shadow-sm'
                        : themeStore.isDarkResolved
                        ? 'bg-green-500/10 border-green-500/20 border-l-green-500 text-green-400'
                        : 'bg-green-100/30 border-green-200/80 border-l-green-600 text-green-700 shadow-sm'
                    "
                  >
                    <div
                      class="flex items-center gap-1.5 font-bold text-[10px]"
                    >
                      <ShieldAlert
                        v-if="payloadCache[item.id].status === 'error'"
                        :size="12"
                      />
                      <Check v-else :size="12" />
                      <span
                        >梦境运行结束: {{ payloadCache[item.id].status }}</span
                      >
                    </div>
                    <div
                      v-if="payloadCache[item.id].error"
                      class="leading-relaxed font-mono text-[9px] select-text whitespace-pre-wrap"
                      :class="
                        themeStore.isDarkResolved
                          ? 'text-white/80'
                          : 'text-gray-650'
                      "
                    >
                      {{ payloadCache[item.id].error }}
                    </div>
                    <div
                      class="leading-relaxed font-mono text-[10px]"
                      :class="
                        themeStore.isDarkResolved
                          ? 'text-white/80'
                          : 'text-gray-750'
                    ...
                    >
                      {{
                        payloadCache[item.id].message || "正常离梦，数据收拢。"
                      }}
                    </div>
                  </div>
                </div>
                ...
                <!-- Agent 梦调度通知 -->
                <div
                  v-else-if="item.type === 'AGENT_DREAM_SCHEDULE'"
                  class="space-y-2.5"
                >
                  <div
                    class="p-2.5 rounded border-l-2 space-y-1.5"
                    :class="
                      themeStore.isDarkResolved
                  ...
                  >
                    ...
                    <div
                      class="text-[9px] font-mono pt-1 border-t"
                      :class="
                        themeStore.isDarkResolved
                          ? 'border-white/5 text-white/45'
                          : 'border-gray-300/40 text-gray-550'
                      "
                    >
                      计划调度成员:
                      {{ payloadCache[item.id].agents?.join(", ") || "None" }}
                    </div>
                  </div>
                </div>
                ...
                <div v-else class="mt-2">
                  <pre
                    class="p-2.5 rounded border overflow-x-auto text-[10px] font-mono leading-relaxed select-text"
                    :class="
                      themeStore.isDarkResolved
                        ? 'bg-[#111620] border-white/5 text-slate-300'
                        : 'bg-[#f1f3f5] border-gray-300/60 text-slate-700'
                    "
                    >{{ JSON.stringify(payloadCache[item.id], null, 2) }}</pre
                  >
                </div>
              </template>
            </div>
      ...
      <!-- 底部栏 -->
      <div
        class="flex items-center justify-between px-4 py-2 border-t relative z-10"
        :class="
          themeStore.isDarkResolved
            ? 'bg-[#1a202c] border-white/5 text-white/40'
            : 'bg-[#edf0f2] border-gray-350/50 text-gray-500'
        "
      >
