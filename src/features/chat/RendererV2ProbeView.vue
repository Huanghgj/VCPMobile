...
const streamBlock = ref<ContentBlock>({
  type: "markdown",
  content:
    '<section id="probe-stream-scene" data-testid="probe-stream-scene"><span data-vcp-key="stream-copy">stream frame 1</span></section>',
});
...
async function onStreamRendered() {
  ...
  if (!streamUpdateScheduled) {
    ...
    firstStreamScene = current;
    window.setTimeout(() => {
      streamBlock.value = {
        type: "markdown",
        content:
          '<section id="probe-stream-scene" data-testid="probe-stream-scene"><span data-vcp-key="stream-copy">stream frame 2</span><b id="probe-stream-new">new node</b></section>',
      };
    }, 50);
    return;
  }
