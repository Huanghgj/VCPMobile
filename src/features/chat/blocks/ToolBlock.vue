  ...
  >
    ...
    >
      <div class="flex items-center gap-2">
        ...
        <div>
          <span
            class="tool-label text-[10px] font-bold block leading-none mb-1 flex items-center gap-1"
          >
      ...
      <div class="flex items-center gap-2">
        <span
          v-if="block.status"
          class="tool-status text-[10px] px-1.5 py-0.5 rounded font-bold"
        >
          {{ block.status }}
        </span>
        <button
    ...
    >
      <template v-if="type === 'tool-use'">
        <pre class="text-[11px] font-mono whitespace-pre-wrap break-words">{{
          content
        }}</pre>
      </template>
      <template v-else>
        <div class="space-y-3">
          <div
            ...
            :class="[
              index > 0
                ? 'border-t border-black/5 dark:border-white/5 pt-3 mt-3'
                : '',
            ]"
          >
            <div class="flex items-center justify-between mb-1.5">
              <span
                class="detail-key font-bold font-mono text-[11px] opacity-80"
                >{{ item.key }}:</span
              >
              <button
                v-if="item.value"
            ...
            <div class="min-w-0">
              ...
              <div
                v-else
                class="text-xs opacity-90 vcp-markdown-block compact-markdown select-text"
                v-html="renderMarkdown(item.value || '')"
              ></div>
            </div>
          </div>
          <div
            v-if="block.footer"
            class="mt-2.5 pt-2.5 border-t border-black/10 dark:border-white/10 text-xs opacity-70 vcp-markdown-block compact-markdown select-text"
            v-html="renderMarkdown(block.footer)"
          ></div>
        </div>
      </template>
    </div>
  ...
  <Teleport to="body">
    <Transition name="fade-slide">
      ...
      >
        <!-- Fullscreen Header -->
        <div
          class="vcp-fullscreen-header flex items-center justify-between pb-3 mb-4 border-b"
        >
          <div class="flex items-center gap-2">
            <span class="text-lg leading-none">📊</span>
            <div>
              <span
                class="text-[10px] font-bold block opacity-60 leading-none mb-1"
              >
        ...
        <div class="flex-1 overflow-y-auto vcp-scrollable space-y-4 pr-1">
          <div
            ...
            :class="[
              index > 0
                ? 'border-t border-black/5 dark:border-white/5 pt-4 mt-4'
                : '',
            ]"
          >
            <div class="flex items-center justify-between mb-2">
              <span class="detail-key font-bold font-mono text-xs opacity-90"
                >{{ item.key }}:</span
              >
              <button
                v-if="item.value"
            ...
            <div class="min-w-0">
              ...
              <div
                v-else
                class="text-sm opacity-95 vcp-markdown-block select-text"
                v-html="renderMarkdown(item.value || '')"
              ></div>
            </div>
          </div>
          ...
          <div
            v-if="block.footer"
            class="mt-4 pt-4 border-t border-black/10 dark:border-white/10 text-xs opacity-70 vcp-markdown-block select-text"
            v-html="renderMarkdown(block.footer)"
          ></div>
        </div>
      </div>
