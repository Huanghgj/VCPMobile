...
<template>
  <div
    class="flex-1 overflow-y-auto overflow-x-hidden vcp-scrollable no-rubber-band bg-transparent pb-6"
  >
    <div
    ...
    >
      <div
        class="w-12 h-12 rounded-full bg-pink-50 flex items-center justify-center border border-pink-100/60 mb-3"
      >
        <Inbox class="w-5 h-5 text-pink-400" />
      </div>
      <p class="text-xs text-slate-500 font-mono font-bold tracking-wider">
        NO NOTIFICATIONS
      </p>
    ...
    <TransitionGroup v-else name="list" tag="div" class="space-y-3">
      ...
      >
        <div
          class="px-4 py-1.5 text-[9px] font-mono font-bold text-pink-500/80 tracking-wider sticky top-0 z-20 bg-[#fff5f7] flex items-center gap-1.5"
        >
        ...
        <div class="mt-1">
          <NotificationCard
            ...
            @action="
              store.executeAction($event.id, $event.action, $event.reason)
            "
          />
        </div>
      </div>
